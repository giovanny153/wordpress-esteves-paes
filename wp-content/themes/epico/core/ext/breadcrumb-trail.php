	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = !empty( $matches ) ? sprintf( '<a itemtype="https://schema.org/Thing" itemprop="item" %s><span itemprop="name">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<a itemtype="https://schema.org/Thing" itemprop="item" href="%s"><span itemprop="name">%s</span></a>', $current_url, $item );
