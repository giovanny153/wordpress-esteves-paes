...
class Widget_Epico_Notice extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'epico_notice-id', __( 'Épico Notice', 'uf-epico' ), array( 'description' => __( 'Create a cool notice at the top of your website.', 'uf-epico' ) ), array( 'width' => 495 ) );
	...
	public function form( $instance ) {
		...
		if ( file_exists( self::get_path( dirname( __DIR__ ) ) . 'configs/fieldgroups-epico_notice.php' ) ) {
			include self::get_path( dirname( __DIR__ ) ) . 'configs/fieldgroups-epico_notice.php';
		} else {
			return;
		}
				...
				if ( is_active_sidebar( $sidebarid ) && $sidebarid != 'wp_inactive_widgets' && false === strpos( $sidebarid, 'orphaned_widgets' ) ) {
					foreach ( $widget_settings as $key => $settings ) {
						if ( in_array( $this->id_base . '-' . $key, $sidebar ) ) {
							...
							wp_enqueue_script( 'epico_notice-epicoaviso', self::get_url( '/assets/js/uberaviso.js', dirname( __DIR__ ) ), array( 'jquery' ), false, true );
						}
					}
				}
			}
